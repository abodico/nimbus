...
const Stages = () => {
    ...
    return (
        <div className='w-full relative py-32  max-w-full' >
            <div className="text-center sm:w-1/2 px-4 mx-auto mb-8">
            ...
            <div className="container mx-auto max-w-full lg:flex-nowrap flex-wrap gap-4 flex justify-evenly my-8 relative">
                {stages.map((item, index) => {
                    return (
                        <div key={index} onMouseEnter={() => setStageHovered({ indx: index })} onMouseLeave={() => setStageHovered({ indx: -1, animate: 'paused' })} className={`portfolio__stages-cards__card  lg:w-72 md:w-2/5 w-full sm:w-4/5 mx-4 md:mx-0 bg-white transition-all duration-300 rounded-lg relative overflow-hidden px-6 pt-24 pb-12 shadow-lg `} >
                            ...
                            <div className=" z-10 text-3xl absolute top-4 left-4 text-dark ">0{index+1}</div>
                            <div className={`bg-stages_background ${stageHovered.indx === index ? ' opacity-100 ' : ' opacity-0'} transition-all duration-300 left-0 absolute w-full h-full top-0 block `}></div>
                            <div className=" mt-1.5 mb-4 relative">
                                ...
                                <p className={`text-base ${stageHovered.indx === index ? 'text-c6' : 'text-61'} `} >{item.text}</p>
                            </div>
                            <a href="ww" rel='noreferrer' className=' flex items-center mt-4 cursor-pointer text-white relative' >
                                <span onMouseEnter={() => setSeeMore(index)} onMouseLeave={() => setSeeMore(-1)} className={`inline-block py-2 px-3 font-light bg-darkYellow transition-all text-xl ${seeMore === index ? 'rounded-l-md' : ' delay-200 rounded-md'} `} >
