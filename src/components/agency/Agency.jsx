...
return (
    <div className='container py-16 px-8'>
        ...
        <div className="grid lg:grid-cols-12 md:grid-cols-8 grid-cols-4 gap-4">
            {agency.map((item, index) => {
                return (
                    <div key={index} className="box lg:w-auto col-span-4 ">
                        <div className="box__inner px-3 relative">
                            ...
                            <div className="box__inner-face w-full overflow-hidden bg-agency_background back absolute top-0 left-0 px-10 xl:py-10 py-6 h-full bg-cover bg-darkYellow rounded-lg ">
                                <h3 className="text-white text-3xl tracking-wide mb-4">{item.title}</h3>
                                <p className="text-white xl:text-lg lg:text-base text-lg leading-loose">These are the words we live by in everything we do. Every story we tell, every brand we build, and every interaction we create must not only look beautiful.</p>
                            </div>
                        </div>
                    </div>
                );})}
